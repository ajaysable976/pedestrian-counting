...
while(cap.isOpened()):
   # Read an image from the video source
            ...
            if cy in range(up_limit, down_limit):
                for i in persons:
                    if abs(x-i.getX()) <= w and abs(y-i.getY()) <= h:
                        ...
                        new = False
                        # update coordinates on object and resets age
    ...
    str_up = 'people_crossed_in: ' + str(cnt_up)
    str_down = 'people_crossed_Out: ' + str(cnt_down)
    str_store = 'In_Store: ' + str(cnt_store)
    frame = cv2.polylines(frame, [pts_L1], False, line_down_color, thickness=2)
    frame = cv2.polylines(frame, [pts_L2], False, line_up_color, thickness=2)
    cv2.putText(frame, str_up, (10, 400), font,
                0.5, (0, 0, 255), 2, cv2.LINE_AA)
    cv2.putText(frame, str_down, (10, 430), font,
                0.5, (255, 0, 0), 2, cv2.LINE_AA)
    cv2.putText(frame, str_store, (10, 460), font,
                0.5, (0, 255, 0), 2, cv2.LINE_AA)
    if cnt_store > 5:
        cv2.putText(frame, 'Warning', (160, 260), font,
